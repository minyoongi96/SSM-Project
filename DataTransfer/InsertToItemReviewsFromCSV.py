...
def convertToReviews(PATH):
    ...
    for item_name in item_names:
        ...
        for i in range(len(temp)):
            ...
            rows.append([item_seq['item_seq'], item_cate, item_rate])
...
if __name__ == "__main__":
    ...
    for file in file_list:
        ...
        for row in rows:
            try:
                sql = f'insert into reviews (item_seq, review_category, review_sentiment, review_star) values ("{row[0]}", "{row[1]}", 0,  {row[2]})'
                db.execute(sql)
            except DataError:
                print(row)
